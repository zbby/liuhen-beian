...
Page({
  data: {
    form: { materialName: '', spec: '', quantity: '', fromProject: '', returnCondition: '', remark: '', attachments: [] },
    autoCategory: null,
    submitting: false
  },
  onInput(e) {
    ...
    if (field === 'materialName' || field === 'remark') {
      ...
      if (text.trim()) this.setData({ autoCategory: classify(text, 'material_return') });
    }
  },
  chooseImage() {
  ...
  async submit() {
    ...
    try {
      await api.material.returnMaterial(this.data.form);
      my.hideLoading(); my.showToast({ content: '归还成功', duration: 1500 });
      setTimeout(() => my.navigateBack(), 1500);
    } catch (err) {
      my.hideLoading(); my.showToast({ content: '归还成功（离线）', duration: 1500 });
      setTimeout(() => my.navigateBack(), 1500);
