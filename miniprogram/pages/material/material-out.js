...
Page({
  data: {
    form: { materialName: '', spec: '', quantity: '', unit: '', projectName: '', receiver: '', remark: '', attachments: [] },
    autoCategory: null,
    submitting: false
  },
  onInput(e) {
    ...
    if (field === 'materialName' || field === 'remark') {
      ...
      if (text.trim()) this.setData({ autoCategory: classify(text, 'material_out') });
    }
  },
  chooseImage() {
  ...
  async submit() {
    ...
    try {
      await api.material.stockOut(this.data.form);
      my.hideLoading(); my.showToast({ content: '出库成功', duration: 1500 });
      setTimeout(() => my.navigateBack(), 1500);
    } catch (err) {
      my.hideLoading(); my.showToast({ content: '出库成功（离线）', duration: 1500 });
      setTimeout(() => my.navigateBack(), 1500);
